...
while (time - 1.00) < 0:
	...
	change_in_skin_temp = (heat_storage_skin*2.0)/therm_cap_skin
	change_in_temp_core = (heat_storage_core*2.0)/therm_cap_core

	# Note unit of time is one hour
	change_in_time = 1.0/60.0

	# To adjust integration over small steps in change_in_skin_temp and change_in_temp_core
	U = abs(change_in_skin_temp)
	if (U*change_in_time - 0.1) > 0:
		change_in_time = 0.1/U
	U = abs(change_in_temp_core)
	if (U*change_in_time - 0.1) > 0:
		change_in_time = 0.1/U
	time = time + change_in_time
	temp_skin_avg = temp_skin_avg + change_in_skin_temp*change_in_time
	...
	if (heat_evap_sweat - 500.0) <= 0:
		...
		STORE = metabolic_rate - heat_evap - DRY - work
		STOREC = STORE*2.0/(mass_body*0.97)
		...
		while E < 0:
			...
			E = rel_humidity - (get_sat_vap_pressure(wet_bulb_temp) - 0.00066*760.0*(temp_air - wet_bulb_temp)*(1.0+0.00115*wet_bulb_temp))/get_sat_vap_pressure(temp_air)
			if E < 0:
		...
		DDEW = -1.0
		while DDEW < 0:
			X = get_sat_vap_pressure(wet_bulb_temp) - (temp_air - wet_bulb_temp)/2.0
			DDEW = (get_sat_vap_pressure(TDEW) - X)
			if DDEW < 0:
				TDEW = TDEW + 0.10
		...
		TOH = (XA*temp_air + wetness*XB*TDEW)/(XA + wetness*XB)
